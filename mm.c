/*
* the mm_malloc function searches for a suitable free block using the next-fit strategy. 
* If a suitable block is found, it marks the block as allocated and returns it. 
* Otherwise, it extends the heap, marks the newly allocated block, and returns it.
 */
...
static void* last_bp;
static __inline__ void* extend_heap(size_t words);
static __inline__ void* coalesce(void* bp);
static __inline__ void *next_fit(size_t adjusted_size);
static __inline__ void mark(void* bp,size_t adjusted_size);
...
static __inline__ void mark(void*bp,size_t adj_size)
{
...
static __inline__ void* next_fit(size_t adj_size)
{
    void* bp=last_bp;
    do
    {
